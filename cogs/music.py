    ...
    async def from_url(cls, query, *, loop=None, requester: discord.User):
        ...
        if len(data['entries']) == 1:
            for entry in data['entries']:
                return Song(discord.FFmpegPCMAudio(entry.get('url'), **cls.ffmpeg_options), data=entry, requester=requester)
...
class Queue:
    ...
    def remove_song_at_index(self, index: int):
        if index <= 0:
            ...
            return f'Removed `{self.entries.pop(index).title}`'

    def move_song(self, song_index: int, new_index: int):
        self.entries.insert(new_index, self.entries.pop(song_index))
        return f'Moved `{self.entries[new_index].title}` to position `{new_index}`'
        ...
        if len(self.entries) == 0:
            ...
            for index, song in enumerate(self.entries):
                if index == 0:
                    ...
                    queued += f'`{index}.` [{song.title}]({song.webpage_url}) | `{song.duration}` | `Requested by {song.requester.name}#{song.requester.discriminator}`\n'
            if len(self.entries) > 1:
                embed.add_field(name='__Enqueued__', value=queued, inline=False)
        return embed
...
class Music(commands.Cog):
    ...
    async def play(self, ctx, *, query):
        ...
        async with ctx.channel.typing():
            ...
            if len(self.queue) == 0:
                empty = True
            sources = await YTDLSource.from_url(query=query, requester=ctx.author)
            if isinstance(sources, Song):
                self.queue.add_song(sources)
            else:
        ...
        if empty:
            ...
            await ctx.send(embed=sources.create_embed())
    ...
    async def queue(self, ctx):
        ...
        if ctx.voice_client is None:
            ...
            await ctx.send(embed=self.queue.create_embed())
